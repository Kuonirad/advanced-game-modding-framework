...
class AtomicModifier:
    ...
    def atomic_patch(self, address: int, new_bytes: bytes,
                    verify_callback: Optional[Callable[[bytes], bool]] = None) -> bool:
        ...
        with self._global_lock:
            ...
            with region_lock:
                try:
                    ...
                    # Simulate atomic write (in real implementation, use OS mechanisms)
    ...
    def release_region(self, address: int) -> None:
        ...
        """
        with self._global_lock:
            print(f"\nBefore release - Active regions: {[hex(addr) for addr in self._active_regions.keys()]}")
            if address in self._region_locks:
                del self._region_locks[address]
                del self._active_regions[address]
            print(f"After release - Active regions: {[hex(addr) for addr in self._active_regions.keys()]}\n")
