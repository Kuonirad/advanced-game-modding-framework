...
class SecurityValidator:
    ...
    def validate_modification(self, address: int, new_bytes: bytes,
                            expected_checksum: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        ...
        if expected_checksum and new_checksum != expected_checksum:
            return False, f"Checksum mismatch: expected {expected_checksum}, got {new_checksum}"
    ...
    def rollback_modification(self, address: int) -> bool:
        ...
        # Clean up tracking
        original_bytes = self._backups.pop(address)
