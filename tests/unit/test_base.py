...
def test_validate_memory_region():
    ...
    assert not framework.validate_memory_region(0x3100, 16)  # Inside page
    assert framework.validate_memory_region(0x4000, 16)  # Next page
    
    # Test boundary conditions
    assert framework.validate_memory_region(0x2F00, 16)  # Just before
    assert not framework.validate_memory_region(0x3FF0, 16)  # Just inside
...
def test_register_hook():
    ...
    # Test hook with None address
    with pytest.raises(ValueError):
        framework.register_hook(None, b"\x90\x90")
